...
const LoginForm = () => {
    ...
    return (
        <form
            className="mt-7 grid space-y-4"
            method="post"
            onSubmit={userLogin}
        >
            {error && (
                <span className="bg-red-100 text-red-500 px-3 py-2.5 rounded-lg text-center transition-all duration-200">
                    {responseStatus}
                </span>
            )}
            <Input
            ...
            <button
                onClick={setDemoUser}
                className="h-12 px-6 rounded-full border border-violet-500 hover:bg-violet-100 text-violet-600 font-semibold cursor-pointer"
            >
                Log in as Demo user
            </button>
        </form>
